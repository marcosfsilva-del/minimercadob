...
from app.features.regra_leve3_pague2.manifest import resumo_economia, selo_item
...
def test_total_final_considera_a_economia():
    itens = carrinho(sabonete(), 3) + carrinho(arroz(), 1)
    promocao = calcular_promocao(itens)

    assert promocao["total_sem_desconto"] == round(PRECO * 3 + 24.90, 2)
    assert promocao["total_final"] == round(promocao["total_sem_desconto"] - PRECO, 2)


def test_resumo_do_carrinho_mostra_o_valor_economizado():
    app = create_app()
    with app.test_request_context():
        html = resumo_economia(items=carrinho(sabonete(), 3), total=PRECO * 3)

    assert "economizou" in html
    assert "2,49" in html


def test_resumo_nao_aparece_quando_a_regra_nao_se_aplica():
    app = create_app()
    with app.test_request_context():
        assert resumo_economia(items=carrinho(sabonete(), 2), total=PRECO * 2) == ""
        assert selo_item(item={"product": sabonete(), "quantity": 2}) == ""
